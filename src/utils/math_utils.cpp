...
auto
spherical_to_cartesian(double theta, double phi, double R, const Eigen::Vector3d& c) -> Eigen::Vector3d
{
...
spherical_to_cartesian(const Eigen::Matrix2Xd& sph, double R, const Eigen::Vector3d& c) -> Eigen::Matrix3Xd
{
  ...
  Eigen::Transform<double, 3, Eigen::Affine> T = Eigen::Translation3d(c) * Eigen::Scaling(R);
